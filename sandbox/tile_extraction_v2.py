...
def mask_tissue(image, kernel_size=(7, 7), gray_threshold=220):
...
def compute_coords(image,
    ...
    image = utils.pad_image(image, patch_size, 'maximum')
    mask = utils.pad_image(mask, patch_size, 'minimum')
    ...
    if len(np.where(x_sum > 0)[0]) > len(np.where(y_sum > 0)[0]):
        image = utils.transpose_image(image)
        mask = utils.transpose_image(mask)
        y_sum, _ = x_sum, y_sum
        transposed = True
    else:
        transposed = False
    ...
    y_tissue_parts_indices = utils.get_tissue_parts_indices(
    ...
    for yidx in y_tissue_parts_indices:
        y_tissue_subparts_coords = utils.get_tissue_subparts_coords(
        ...
        for y in y_tissue_subparts_coords:
            ...
            x_tissue_parts_indices = utils.get_tissue_parts_indices(
            ...
            for xidx in x_tissue_parts_indices:
                x_tissue_subparts_coords = utils.get_tissue_subparts_coords(
                    xidx, patch_size, min_decimal_keep)
                
                for x in x_tissue_subparts_coords:
                    coords = utils.eval_and_append_xy_coords(
